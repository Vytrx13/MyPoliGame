...
export default function ListaContainer({
      ...
      const fetchListGames = async () => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            setError(
              errorData.message ||
                `Erro ${res.status} ao buscar jogos da lista.`
            );
    ...
    try {
      ...
      if (res.ok) {
        ...
        switch (res.status) {
          ...
          case 404:
            setError(
              "Detalhes do jogo não encontrados (pode ter sido removido da API externa)."
            );
            break;
          default:
  ...
  // Error State
  if (error && !isGameSelected) {
    // Show list-related errors only when viewing the list
    ...
    if (games.length === 0) {
      return (
        <div className="empty-message">
          ...
          {tipoLista ? `na lista "${tipoLista}"` : "nas listas"} de {donoLista}.
        </div>
      );
    }
    ...
    return (
      <div className="games-container">
        ...
        <h3 className="games-title">
          {tipoLista ? `Lista "${tipoLista}"` : "Todas as Listas"} de{" "}
          {donoLista}
        </h3>
        <div className="games-grid">
          {games.map((game) => (
            <div
              ...
              tabIndex={0}
              onKeyDown={(e) =>
                (e.key === "Enter" || e.key === " ") && handleGameSelect(game)
              }
            >
